...
export const s = {
  ...
  row: (kind: "attached" | "inherited" | "available", dragging: boolean): CSSProperties => ({
    display: "flex",
    alignItems: "center",
    gap: 10,
    padding: "9px 12px",
    borderRadius: 7,
    border: "1px solid var(--border)",
    // Inherited rows are dashed so they read as "arrives through a skill",
    // not as something this agent chose.
    borderStyle: kind === "inherited" ? "dashed" : "solid",
    /* Highlight means ONE thing: this row's checkbox is ticked. Inherited rows
       used to share the highlight, so rows the human never touched looked
       selected — reported 2026-08-23. They stay on the plain background and are
       told apart by their dashed border instead. */
    background: kind === "attached" ? "var(--bg-hover)" : "var(--bg-elevated)",
    opacity: dragging ? 0.7 : kind === "attached" ? 1 : 0.85,
    width: "100%",
