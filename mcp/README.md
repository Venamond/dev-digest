...
| `get_blast_radius` | Maps which files and symbols a pull request impacts, and who calls them. Requires the repository to be indexed by DevDigest first. | **L04**'s Blast Radius over MCP (`GET /pulls/:id/blast`, `server/src/modules/blast/routes.ts`) — zero LLM calls by default, read straight from the persistent code index. Pass `summary: true` to also get a one-paragraph explanation of the map — that branch makes **one** LLM call and costs money, which is why this tool carries no `readOnlyHint`; the paragraph is generated from the map's nodes alone and is never persisted. If it fails (rate limit, no provider) the map still comes back, with `summary_error`; on an empty or unindexed map no call is made and `summary_skipped` says why. Returns `state`, `totals` and, per changed symbol, its callers, endpoints and crons; `prior_pulls` and `link` stay in the studio. A repo that is not indexed (or was indexed by an older indexer) comes back as a **success** payload with `state: 'degraded'` / `reason: 'index_stale'` and a `hint` pointing at *Re-analyze* — an empty map means "we cannot see", never "nothing is impacted". |
